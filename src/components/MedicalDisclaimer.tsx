 ...
 * kodu, więc widzi go też recenzent sklepu) oraz w module Kontrola, który zbiera
 * tętno i saturację. Zmiana treści = zmiana w tym jednym miejscu.
 */
export function MedicalDisclaimer({ className = '' }: { className?: string }) {
  return (
    <div
      className={`flex items-start gap-2 rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2.5 ${className}`}
    >
